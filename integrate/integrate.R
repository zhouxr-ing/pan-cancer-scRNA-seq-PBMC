...
cat("\n\n----------train test----------\n")
train<-lapply(allData,function(x){x[,1:round(dim(x)[2]/2)]})
test<-lapply(allData,function(x){x[,(round(dim(x)[2]/2)+1):dim(x)[2]]})
for(i in 1:length(train)){
  print(dim(train[[i]]))
  print(dim(test[[i]]))
}
save(train,file="result/mergeGEO/train.Rdata")
save(test,file="result/mergeGEO/test.Rdata")

library(Seurat)
cat("\n\n----------train set Seurat pipeline----------\n")
pbmc.list<-list()
for(i in 1:length(train)){
  pbmc.list[[i]]<-CreateSeuratObject(train[[i]])
  pbmc.list[[i]]<-SCTransform(pbmc.list[[i]],verbose = FALSE)
  print(i)
}
save(pbmc.list,file="result/mergeGEO/pbmc.list.Rdata")
print("000")
...
print("444")
save(pbmc.integrated,file="result/mergeGEO/pbmc.integrated2.Rdata")

pbmc.integrated<-RunPCA(pbmc.integrated)
pbmc.integrated<-RunTSNE(pbmc.integrated,check_duplicates=F)

pdf("result/mergeGEO/integrated_tsne.pdf")
